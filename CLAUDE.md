...
Before relying on any Bot API detail — a field name, a length limit, an error
string — check `docs/telegram-notes.md`. It is the short page of limits and
behaviours that actually bite this bot, and it points at the two reference
sources: `docs/vendor/telegram-bot-api.md` (an offline, greppable mirror of
<https://core.telegram.org/bots/api>, regenerated by
`python3 scripts/fetch_telegram_docs.py`) and the pinned wrapper's own source
in the module cache.

The mirror is generated and may lag the website; the website wins on any
disagreement. It also documents a much newer Bot API than the pinned
`telegram-bot-api/v5 v5.5.1` implements, so confirm a field exists in the
